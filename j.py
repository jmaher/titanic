...
def updateJob(job, delta=30):
    ...
    retVal = 'updated'
    if (buildCount > 15 or revCount > 15):
        retVal = 'bounds-error'
        print "bounds error coming up: buildcount %s, revcount %s" % (buildCount, revCount)
        ...
        if not (job['buildrevs'] == ''):
            ...
            for rev in buildList:
                if not (titanic.isBuildPending(job['branch'], job['buildername'], rev, auth) \
                    ...
                    print "going to trigger build for %s, %s, %s" % (rev, job['branch'], job['buildername'])
                    buildlist.append(rev)
        ...
        if buildFlag:
            ...
            for rev in revList:
                ...
                if 'talos' in job['buildername']:
                    print "going to trigger job for %s, %s, %s" % (rev, job['branch'], job['buildername'])
#                    titanic.triggerJob(job['branch'], job['buildername'], rev, auth)
#                    titanic.triggerJob(job['branch'], job['buildername'], rev, auth)
...
def processCron():
    ...
    for job in jobs['jobs']:
        buildlist = processJob(job)
        print "%s: %s: missing %s builds: %s" % (job['buildername'], job['revision'], len(buildlist), buildlist)
